// src/pages/history.tsx (admin)
import { useState, useMemo, useEffect } from "react";
import { useQuery } from "@tanstack/react-query";
import { useListAccounts } from "@workspace/api-client-react";
import { fetchHistory, fetchHistorySymbols, type HistoryQueryParams, type HistoryRow } from "@/lib/historyApi";
import { RefreshCw, Search, X, ChevronDown, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Filter } from "lucide-react";

const EVENT_TYPE_OPTIONS: { value: HistoryRow["eventType"] | "ALL"; label: string }[] = [
...
export default function History() {
  ...
  const [accountId, setAccountId] = useState<string>("ALL"); // "ALL" or account id as string
  ...
  useEffect(() => { setPage(1); }, [search, accountId, symbol, side, eventType, dateFrom, dateTo, minQty, maxQty, minRepunchCount, maxRepunchCount, batchId, pageSize]);

  // Account lookup for rendering "who" + building the filter/dropdown.
  const accountMap = useMemo(() => {
    const map = new Map<number, { name: string; mobileNumber: string }>();
    for (const a of accounts ?? []) map.set(a.id, { name: a.name, mobileNumber: a.mobileNumber });
    return map;
  }, [accounts]);
  ...
  const accountOptions = useMemo(
    () => [
      ...
      ...(accounts ?? []).map((a) => ({ value: String(a.id), label: `${a.name} (${a.mobileNumber})` })),
    ],
    [accounts],
  );

  const selectedAccountId = accountId !== "ALL" ? Number(accountId) : undefined;

  const { data: symbols } = useQuery({
    queryKey: ["adminHistorySymbols", selectedAccountId],
    queryFn: () => fetchHistorySymbols(selectedAccountId),
  });
  ...
  const queryParams: HistoryQueryParams = {
    ...
    eventType: eventType !== "ALL" ? (eventType as HistoryRow["eventType"]) : undefined,
  ...
  const activeFilterCount =
    (accountId !== "ALL" ? 1 : 0) + (symbol !== "ALL" ? 1 : 0) + (side !== "ALL" ? 1 : 0) + (eventType !== "ALL" ? 1 : 0) +
    (dateFrom ? 1 : 0) + (dateTo ? 1 : 0) + (minQty ? 1 : 0) + (maxQty ? 1 : 0) +
    (minRepunchCount ? 1 : 0) + (maxRepunchCount ? 1 : 0) + (batchId ? 1 : 0) + (search ? 1 : 0);

  const clearFilters = () => {
    setSearch(""); setAccountId("ALL"); setSymbol("ALL"); setSide("ALL"); setEventType("ALL");
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-xl font-bold">Trade History — All Accounts</h1>
          <p className="text-sm text-muted-foreground">Every auto-trade event across every connected account.</p>
        </div>
      ...
      <div className="rounded-xl p-3 mb-3 space-y-2.5" style={{ border: "1px solid hsl(var(--border))", background: "hsl(var(--card))" }}>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <FilterChip value={accountId} options={accountOptions} onChange={setAccountId} activeColor="hsl(200 88% 55%)" />
          ...
          <tbody>
            {rows.length === 0 ? (
              ...
              <tr key={row.id} style={{ borderBottom: "1px solid hsl(var(--border))", background: idx % 2 === 0 ? "transparent" : "hsl(var(--muted) / 0.3)" }}>
                ...
                <td className="px-3 py-2 font-medium whitespace-nowrap" title={accountMap.get(row.accountId)?.mobileNumber}>{accountLabel(row.accountId)}</td>
